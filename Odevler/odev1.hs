...
* Dosyayı çalıştırdığınızda bazı test case'ler çalışacak ve olası puanınızı ekrana yazacaktır. Ancak bu puan son puanınız olmayabilir. Ödeviniz değerlendirilirken başka test case'ler kullanılacaktır. Buradaki test case'lerin amacı size yardımcı olmaktır.
...
* Sorular kolay ve orta seviye zorluktadır. Yapabildiğiniz kadarını yapmaya çalışınız.
