...
printSekil (Circ (x, y) r) = show x ++ ", " ++ show y ++ " merkezli, alani " ++ 
  show (alanHesapla (Daire r)) ++ " olan bir daire"

printSekil (Rect (x0, y0) (x1, y1)) = show x0 ++ ", " ++ show y0 ++ " ve " ++
  show x1 ++ ", " ++ show y1 ++ " koordinatlarinda, alani " ++ 
  show (alanHesapla (Dikdortgen (abs (x0 - x1)) (abs (y0 - y1)))) ++ " olan bir dikdortgen"
